...
trait Columns
{
    ...
    */
    ...
    {
        // check following request() fields to get a list fields requested
        $requestCsvKeys = [
            'columns',
            'columns_csv',
            'fields',
    ...
    */
    ...
    public function aliasColumns()
    {
        ...
        if (request('alias_columns_csv')) {
            $aliases = Convert::csvToArray(request('alias_columns_csv'));
        } else { // Else, copy all the selected column values for alias.
            $aliases = $this->selectedColumns();
        }

        // If number of alias is less than selected columns then fill the rest
        // from the selected columns.
        if (count($aliases) <= count($this->selectedColumns())) {
            $aliases = $this->fillAliasColumns($aliases);
        }

        // If alias count is more than selection then trim
        if (count($aliases) > count($this->selectedColumns())) {
            $aliases = array_slice($aliases, 0, count($this->selectedColumns()));
        }

        $aliases = $this->removeDotFromColumns($aliases);
