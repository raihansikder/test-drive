...
class ModelProcessor
{
	 ...
	 * fields that are not filled through mass assignment but needs to be
		...
		foreach ($allTransitions as $field => $transition) {
			if ($this->element->fieldHasChanged($field)) {
				...
				if ($change && !$this->transitionIsAllowed($field, $change['old'], $change['new'])) {
					$this->fieldError($field,
						ucfirst($field)." - can not be updated from  '".$change['old']."' to '".$change['new']."'");
				}
			}
		}
	...
	*/
	...
	public function save()
	{
		...
		if (!$this->isValid()) { // If invalid, add message to message bag
	...
	*/
	...
	/**
	 * Propagate data from one table to another if on of the given fields
	 * has changed.
	 *
	 * @param  array  $fields
	 * @return void
	...
	*/
	/**
	 * Check if the name is duplicate within a tenant. Same name can be used by
	 * other tenants
	 *
	 * @return $this
	 */
