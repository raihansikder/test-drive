...
{
    // Case 1: If input is an array, return it
    if (is_array($input)) {
        return $input;
    }

    // Case 2: If input is a string, convert it to an array
    if (is_string($input)) {
        // Case 2.1: If input is JSON, convert it to an array
        if (isJson($input)) {
            return json_decode($input, true);
        }

        // Case 2.2: If the string is CSV '1,2,3' or array expression '[1,2,3]' convert it to an array
        return Convert::strToArray($input);
    }

    // Case 3: By default, if the input has a value, then wrap in array else return null
...
/**
 * gets the total number of days between two dates
 *
 * @param  string  $date1
 * @param  string  $date2
...
{
    if (in_array(strtolower($ext), ['jpg', 'png', 'gif', 'jpeg', 'tiff', 'bmp'])) {
        return true;
    }
