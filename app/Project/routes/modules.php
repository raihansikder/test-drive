...
use App\Project\Modules\ModuleGroups\ModuleGroupController;
...
Route::middleware(['auth', 'verified', 'tenant'])->group(function () use ($modules, $moduleGroups) {
    ...
    Route::get('show-image/{id}', [UploadController::class, 'showImage'])->name('show.image'); // Deprecated. use the show.upload route
    Route::get('show-upload/{id}', [UploadController::class, 'showUpload'])->name('show.upload');
